...
def visualize_specific_scenes(scene_names, output_folder):
    ...
    viser = Visualizer(
        ...
        predroot='output/results_10_converted.pkl', 
...
if __name__ == '__main__':
    # Just visualize the first few samples from results_10.pkl
    ...
    output_folder = 'output/results_10_viz'
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)
        
    print(f"Found {len(viser.token_set)} samples with predictions")
    
    # Visualize first 10 samples 
    for i, sample_token in enumerate(list(viser.token_set)[:10]):
        try:
            ...
            output_path = os.path.join(output_folder, f"sample_{i:03d}_{scene_name}")
            
            print(f"Processing sample {i}: {scene_name} (token: {sample_token[:16]}...)")
    ...
    print(f"Visualization complete! Check {output_folder} for results.")