...
class PansegformerHead(SegDETRHead):
    ...
    def forward_test(self,
        ...
        with torch.no_grad():
            ...
            # Check if gt_lane_masks has valid dimensions
            if gt_lane_masks[0].shape[1] > 0:
                ...
                # Handle empty gt_lane_masks - create dummy tensors with zero values
                print("Warning: gt_lane_masks is empty, using dummy values for evaluation")
                drivable_gt = torch.zeros_like(drivable_pred)
                drivable_iou, drivable_intersection, drivable_union = torch.tensor(0.0), torch.tensor(0.0), torch.tensor(0.0)

                lane_pred = results[0]['lane']
                lanes_pred = (results[0]['lane'].sum(0) > 0).int()
                lanes_gt = torch.zeros_like(lanes_pred)
                lanes_iou, lanes_intersection, lanes_union = torch.tensor(0.0), torch.tensor(0.0), torch.tensor(0.0)

                divider_gt = torch.zeros_like(lanes_pred)
                crossing_gt = torch.zeros_like(lanes_pred)
                contour_gt = torch.zeros_like(lanes_pred)
    ...
    def get_bboxes(
        ...
        for img_id in range(len(img_metas)):
            ...
            pts_filename = img_metas[img_id]['pts_filename']
            if pts_filename is not None:
                file_name = pts_filename.split('/')[-1].split('.')[0]
            else:
                # Use img_id as fallback when pts_filename is None
                file_name = f"sample_{img_id}"
