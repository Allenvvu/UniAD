...
class NuScenesE2EDataset(NuScenesDataset):
    ...
    def get_ann_info(self, index):
        ...
        # Handle different data structures for L2G converted data
        if isinstance(info['gt_boxes'], list):
            # Convert lists to numpy arrays for indexing
            gt_boxes_array = np.array(info['gt_boxes']) if len(info['gt_boxes']) > 0 else np.array([]).reshape(0, 7)
            gt_names_array = np.array(info['gt_names']) if len(info['gt_names']) > 0 else np.array([])
            gt_inds_array = np.array(info.get('gt_inds', [])) if len(info.get('gt_inds', [])) > 0 else np.array([])
            
            if self.use_valid_flag:
                # For L2G data, valid_flag is a single boolean, not an array
                mask = np.ones(len(gt_boxes_array), dtype=bool) if info['valid_flag'] else np.zeros(len(gt_boxes_array), dtype=bool)
            else:
                # For L2G data, num_lidar_pts is a single integer, not an array
                mask = np.ones(len(gt_boxes_array), dtype=bool) if info['num_lidar_pts'] > 0 else np.zeros(len(gt_boxes_array), dtype=bool)
        else:
            ...
            gt_boxes_array = info['gt_boxes']
            gt_names_array = info['gt_names']
            gt_inds_array = info['gt_inds']
        
        gt_bboxes_3d = gt_boxes_array[mask] if len(gt_boxes_array) > 0 else np.array([]).reshape(0, 7)
        gt_names_3d = gt_names_array[mask] if len(gt_names_array) > 0 else np.array([])
        gt_inds = gt_inds_array[mask] if len(gt_inds_array) > 0 else np.array([])
        ...
        if self.with_velocity:
            # Handle different data structures for L2G converted data
            if isinstance(info['gt_velocity'], list):
                gt_velocity_array = np.array(info['gt_velocity']) if len(info['gt_velocity']) > 0 else np.array([]).reshape(0, 2)
                gt_velocity = gt_velocity_array[mask] if len(gt_velocity_array) > 0 else np.array([]).reshape(0, 2)
            else:
                gt_velocity = info['gt_velocity'][mask]
            
            if len(gt_velocity) > 0:
    ...
    def get_data_info(self, index):
        ...
        for i, gt_mask in enumerate(map_mask[:-1]):
            ys, xs = np.where(gt_mask)
            if len(xs) > 0 and len(ys) > 0:  # Only process if there are non-zero elements
                gt_bbox = [min(xs), min(ys), max(xs), max(ys)]
                gt_labels.append(i + self.map_num_classes)
                gt_bboxes.append(gt_bbox)
                gt_masks.append(gt_mask)
        if len(gt_labels) > 0:  # Only create tensors if there are valid masks
            gt_labels = torch.tensor(gt_labels)
            gt_bboxes = torch.tensor(np.stack(gt_bboxes))
            gt_masks = torch.stack(gt_masks)
        else:  # Create empty tensors if no valid masks found
            gt_labels = torch.tensor([], dtype=torch.long)
            gt_bboxes = torch.tensor([], dtype=torch.float32).reshape(0, 4)
            gt_masks = torch.tensor([], dtype=torch.bool).reshape(0, *map_mask.shape[1:])

        # standard protocal modified from SECOND.Pytorch
        input_dict = dict(
            sample_idx=info['token'],
            pts_filename=info.get('lidar_path', None),  # Handle missing lidar_path for camera-only inference
            sweeps=info['sweeps'],
        ...
        if self.modality['use_camera']:
            ...
            for cam_type, cam_info in info['cams'].items():
                ...
                # Handle case where no lidar data is available (camera-only inference)
                if 'sensor2lidar_rotation' in cam_info and 'sensor2lidar_translation' in cam_info:
                    lidar2cam_r = np.linalg.inv(cam_info['sensor2lidar_rotation'])
                    lidar2cam_t = cam_info['sensor2lidar_translation'] @ lidar2cam_r.T
                else:
                    # Use identity transformation when no lidar data is available
                    lidar2cam_r = np.eye(3)
                    lidar2cam_t = np.zeros(3)
    ...
    def occ_get_detection_ann_info(self, index):
        ...
        # Handle different data structures for L2G converted data
        if isinstance(info['gt_boxes'], list):
            gt_bboxes_3d = np.array(info['gt_boxes']) if len(info['gt_boxes']) > 0 else np.array([]).reshape(0, 7)
            gt_names_3d = np.array(info['gt_names']) if len(info['gt_names']) > 0 else np.array([])
        else:
            gt_bboxes_3d = info['gt_boxes'].copy()
            gt_names_3d = info['gt_names'].copy()
        
        gt_ins_inds = info.get('gt_inds', []).copy() if 'gt_inds' in info else []
        ...
        if self.with_velocity:
            # Handle different data structures for L2G converted data
            if isinstance(info['gt_velocity'], list):
                gt_velocity = np.array(info['gt_velocity']) if len(info['gt_velocity']) > 0 else np.array([]).reshape(0, 2)
            else:
