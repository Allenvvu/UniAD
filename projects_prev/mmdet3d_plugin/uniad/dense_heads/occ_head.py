...
# original code
    ...
    def merge_queries(self, outs_dict, detach_query_pos=True):
        traj_query = outs_dict.get('traj_query', None)
        track_query = outs_dict.get('track_query', None)
        track_query_pos = outs_dict.get('track_query_pos', None)

        # Handle None safely
        if traj_query is None:
            traj_query = torch.zeros_like(track_query)  # fallback
        if track_query is None:
            track_query = torch.zeros_like(traj_query)
        if track_query_pos is None:
            track_query_pos = torch.zeros_like(traj_query)

        if detach_query_pos:
            track_query_pos = track_query_pos.detach()

        # Keep temporal dimension consistent
        if traj_query.ndim == 5:  # [B, T, Q, 1, C] -> remove singleton
            traj_query = traj_query.squeeze(3)
        elif traj_query.ndim == 4:  # [B, T, Q, C] -> OK
            pass
        else:  # [B, Q, C] -> add temporal
            traj_query = traj_query.unsqueeze(1)  # [B, 1, Q, C]

        if track_query.ndim == 3:  # [B, Q, C] -> add temporal
            track_query = track_query.unsqueeze(1)
        if track_query_pos.ndim == 3:
            track_query_pos = track_query_pos.unsqueeze(1)

        # Apply fusers
        ins_query = traj_query[:, -1:]  # last temporal frame, keep T dim
        ins_query = self.mode_fuser(ins_query).max(2)[0]  # [B, T, C]

        # Make sure ins_query has 4D for concatenation
        if ins_query.ndim == 3:
            ins_query = ins_query.unsqueeze(2)  # [B, T, 1, C]

        # Debug: print shapes before alignment
        print(f"DEBUG merge_queries - ins_query shape: {ins_query.shape}")
        print(f"DEBUG merge_queries - track_query shape: {track_query.shape}")
        print(f"DEBUG merge_queries - track_query_pos shape: {track_query_pos.shape}")

        # Handle batch dimension mismatch: take last decoder layer for ins_query
        if ins_query.shape[0] > track_query.shape[0]:
            ins_query = ins_query[-1:, :, :, :]  # Take last decoder layer [1, T, Q, C]
            
        # Align batch dimensions by expanding track tensors
        target_b = ins_query.shape[0]
        if track_query.shape[0] != target_b:
            track_query = track_query.expand(target_b, -1, -1, -1)
        if track_query_pos.shape[0] != target_b:
            track_query_pos = track_query_pos.expand(target_b, -1, -1, -1)
        
        # Handle query dimension mismatch: expand ins_query to match track queries
        target_q = track_query.shape[2]
        if ins_query.shape[2] == 1 and target_q > 1:
            ins_query = ins_query.expand(-1, -1, target_q, -1)  # Expand singleton to match
        elif ins_query.shape[2] != target_q:
            # If dimensions don't match and ins_query isn't singleton, slice or pad as needed
            if ins_query.shape[2] > target_q:
                ins_query = ins_query[:, :, :target_q, :]  # Slice to match
            else:
                # Pad with zeros to match target_q
                padding_needed = target_q - ins_query.shape[2]
                padding = torch.zeros(ins_query.shape[0], ins_query.shape[1], padding_needed, ins_query.shape[3], 
                                    device=ins_query.device, dtype=ins_query.dtype)
                ins_query = torch.cat([ins_query, padding], dim=2)
            
        print(f"DEBUG merge_queries - aligned ins_query shape: {ins_query.shape}")
        print(f"DEBUG merge_queries - aligned track_query shape: {track_query.shape}")
        print(f"DEBUG merge_queries - aligned track_query_pos shape: {track_query_pos.shape}")

        # Concatenate along last dimension
        ins_query = torch.cat([ins_query, track_query, track_query_pos], dim=-1)  # [B, T, Q, C_concat]
        print(f"DEBUG merge_queries - concatenated shape: {ins_query.shape}")

        # Final multi-query fusion
        ins_query = self.multi_query_fuser(ins_query)
        
        # Remove temporal dimension to match expected output format [b, q, c]
        if ins_query.ndim == 4:
            ins_query = ins_query.squeeze(1)  # Remove singleton temporal dimension
        
        print(f"DEBUG merge_queries - final output shape: {ins_query.shape}")
