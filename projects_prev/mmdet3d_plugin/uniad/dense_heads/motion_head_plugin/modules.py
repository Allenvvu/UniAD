...
class MotionTransformerDecoder(BaseModule):
    ...
    def forward(self,
        ...
        print(f"DEBUG: MotionFormer - agent_level_embedding device: {agent_level_embedding.device}")
        print(f"DEBUG: MotionFormer - track_query device: {track_query.device}")
        print(f"DEBUG: MotionFormer - track_query_pos device: {track_query_pos.device}")
        print(f"DEBUG: MotionFormer - lane_query device: {lane_query.device}")
        print(f"DEBUG: MotionFormer - bev_embed device: {bev_embed.device if bev_embed is not None else 'None'}")
        ...
        for lid in range(self.num_layers):
            ...
            # ITRI integration debug logging and validation:
            print(f"DEBUG: BEV interaction layer {lid}")
            print(f"DEBUG: Query shape (agents): {query_embed.shape}")  
            print(f"DEBUG: Value shape (BEV): {bev_embed.shape}")
            print(f"DEBUG: Query tensor dimensions: {query_embed.dim()}D")
            print(f"DEBUG: BEV tensor dimensions: {bev_embed.dim()}D")
            
            # ITRI integration validation:
            if bev_embed.dim() != 3:
                raise ValueError(f"BEV embed should be 3D tensor [B, N, D], got {bev_embed.dim()}D with shape {bev_embed.shape}")
            if query_embed.dim() != 4:
                raise ValueError(f"Query embed should be 4D tensor [B, A, P, D], got {query_embed.dim()}D with shape {query_embed.shape}")
            
            print(f"DEBUG: Expected attention matrix size: {query_embed.shape[1] * query_embed.shape[2]} × {bev_embed.shape[1]} = {query_embed.shape[1] * query_embed.shape[2] * bev_embed.shape[1]} elements")
            print(f"DEBUG: About to call BEV interaction layer with 4D→3D tensor handling...")
            ...
            print(f"DEBUG: BEV interaction layer {lid} completed successfully")
            print(f"DEBUG: Output BEV query embed shape: {bev_query_embed.shape}")
